...
/*
 *  FILE: lost_item_form.php
 *
 * This is the controller class for the public lost item form.
 * It handles both Creation and Update of a lost item record.
 *
 */

class lost_item_form extends secure_page {

	function lost_item_form()
	{
	    /*
		...
		// This page is "Create" lost item by default.
		...
		if ($is_update_form)
			$form_title = "Update lost item";
		else
		 	$form_title = "Post Lost Item";
		...
		if ($do_create)
		{
			...
			$reward = make_safe($_POST['reward']);
			$address = make_safe($_POST['address']);
			$city = make_safe($_POST['city']);
			$state = make_safe($_POST['state']);
			$zip = make_safe($_POST['zip']);
			$user_id = $_SESSION['user_id'];
			$lost_day = make_safe($_POST['lost_day']);
			$lost_month = make_safe($_POST['lost_month']);
			$lost_year = make_safe($_POST['lost_year']);
			$lost_found_date = "{$lost_year}-{$lost_month}-{$lost_day}";
		...
		// the master template...
		$content = $site_globals['TEMPLATE_PATH']."/lost_item_form.template.php";
